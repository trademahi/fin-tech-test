    ...
    const handleGetFinData = async () => {
      try {
        ...
        const response = await axios.get(
          `/api/financialList?page=${currentPage}&perPage=${itemsPerPage}`
        );
        setTimeout(() => {
    ...
    <div className="w-full h-screen bg-white  text-black border relative">
      ...
      <div className="w-full flex  text-black  absolute bottom-[1rem] left-0">
        <div className="w-fit mx-auto flex gap-4">
          ...
          >
            <GrCaretPrevious className="text-[1.5rem] paginationBtn" />
          </button>
          {totalPages === 0 ? (
            <p>0</p>
          ) : (
            <p>
              {currentPage} -{Math.round(totalPages / itemsPerPage)}
            </p>
          )}
          <button
            disabled={currentPage == Math.round(totalPages / itemsPerPage)}
            onClick={() => handlePagination(1)}
          >
            <GrCaretNext className="text-[1.5rem] paginationBtn" />
          </button>
        </div>
      </div>
    </div>
